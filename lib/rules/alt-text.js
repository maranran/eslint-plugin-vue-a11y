...
function hasLabel(node) {
  const ariaLabelProp = VueUtils.getAttribute(node, 'aria-label')
  const arialLabelledByProp = VueUtils.getAttribute(node, 'aria-labelledby')
...
const ruleByElement = {
  img(context, node) {
    ...
    const value = altValue && altValue.value
    if (value || value === '') {
      return
    }
  ...
  object(context, node) {
    const hasLabel = hasLabel(node)
    const titleProp = VueUtils.getAttribute(node, 'title')
    const hasTitleAttr = !!titleProp

    if (hasLabel || hasTitleAttr || utils.hasAccessibleChild(node)) {
      return
    }
  ...
  area(context, node) {
    ...
    const altValue = utils.getAttributeValue(node, 'alt') // <object alt />
    const value = altValue && altValue.value
